...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood[which(bollywood$Tcollection == max(bollywood$Tcollection[bollywood$Tcollection!=max(bollywood$Tcollection)])),1]
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  bollywood[sapply(bollywood[,4:7],which.max),1]  
  movie_result <- bollywood[sapply(bollywood[,4:7],which.max),1]
